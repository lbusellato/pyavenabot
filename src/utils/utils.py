...
def insert_player(conn, tgid, lid):
    tid = get_max_tid() + 1
    elo = get_elo(lid)
    conn.execute("INSERT INTO Partecipanti(tid,lid,tgid,ELO) VALUES('" + str(tid) + "','" + str(lid) + "','" + str(tgid) + "','" + str(elo) + "');")


def lid_exists(lid):
    url = "https://lichess.org/@/" + lid
    try:
